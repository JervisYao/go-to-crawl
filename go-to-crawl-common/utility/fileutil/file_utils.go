...
func DownloadToReaderByBuilder(downloadBuilder *DownloadBuilder) io.ReadCloser {
	...
	// 获取资源
	log.Infof("DownloadToReaderByBuilder: method = %v, url = %v, body = %v", downloadDO.method, downloadDO.url, downloadDO.body)

	body := strings.NewReader(downloadDO.body)
	request, err := http.NewRequest(downloadDO.method, downloadDO.url, body)
	if err != nil || request == nil {
		log.Errorf("err = %v, request = %v", err, request)
	}
	...
	if err != nil {
		...
		log.Infof("DownloadToReaderByBuilder: %v|%v ms|%v bytes|%s", resp.StatusCode, spend, resp.ContentLength, downloadDO.url)
		return resp.Body
	}
}
