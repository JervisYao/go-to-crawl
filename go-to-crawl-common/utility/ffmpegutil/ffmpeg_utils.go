...
import (
	...
	"easygoadmin/appnewcms/utils/httputil"
	"errors"
	"fmt"
...
func DownloadToMp4(m3u8DO *M3u8DO) error {
	log := g.Log().Line()
	log.Infof("final path : %s", m3u8DO.FromDir)
...
func RunFfmpegGenericMerge(basePath string) error {
	...
	g.Log().Infof("待合并文件数: %d", len(orgFiles))
...
func MergeTsFile(m3u8DO *M3u8DO) error {
	...
	err := RunFfmpegCommand(args...)
	if err != nil {
		log.Infof("转换出错->%v", err)
		return err
	}
	log.Info("转换成MP4成功：", mp4File)
	return nil
}
...
func DeleteTmpResource(m3u8DO *M3u8DO, tsFilePattern string) {
	...
	// 删除依赖资源文件
	log.Infof("删除Map文件: %s", m3u8DO.MapSaveFile)
	log.Infof("删除Key文件: %s", m3u8DO.KeySaveFile)
	...
	log.Info("删除流片段个数：", len(streamFiles))
