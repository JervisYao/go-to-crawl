...
import (
	"easygoadmin/appnewcms/service/browsermobservice"
	"easygoadmin/appnewcms/service/crawl/sysservice"
	"easygoadmin/appnewcms/utils/constant"
	"easygoadmin/appnewcms/utils/fileutil"
	"easygoadmin/appnewcms/utils/timeutil"
	"fmt"
...
func GetHarResponseBody(harRequest *gjson.Json) string {
	...
	rspBody := harRequest.GetString(ResponseBody)
...
func printTargetUrl(targetRequestArray []*gjson.Json) {
	for _, targetRequest := range targetRequestArray {
		...
		if targetRequest != nil {
			targetUrl = targetRequest.GetString("url")
		}
		g.Log().Line().Info("代理获取的目标 URL: ", targetUrl)
	}
}

func initHeaderFromHarItem(builder *fileutil.DownloadBuilder, reqJson *gjson.Json) *fileutil.DownloadBuilder {
	headers := reqJson.GetMaps("headers")
	for _, headerMap := range headers {
		headerJson := gjson.New(headerMap)
		key := headerJson.GetString("name")
		value := headerJson.GetString("value")
